...
class MySearchEngine:
    ...
    def extract_tfidf_scores(self, sentence):
        processed_sentence = self.preprocess_text(sentence)
        sentence_vector = self.tfidf_vectorizer.transform([processed_sentence])
        ...
        if similar_indices.any():
            similar_document_idx = similar_indices[0]
            document_vector = self.tfidf_matrix[similar_document_idx]

            # Debugging: Print intermediate results
            print("Processed Sentence:", processed_sentence)
            print("Feature Names:", self.tfidf_vectorizer.get_feature_names_out())
            print("Document Vector:", document_vector.toarray()[0])

            feature_names = self.tfidf_vectorizer.get_feature_names_out()
            tfidf_scores = document_vector.toarray()[0]

            # Filter out features with TF-IDF score of 0.0
            non_zero_tfidf_scores = [(feature, score) for feature, score in zip(feature_names, tfidf_scores) if score != 0.0]

            # Sort the non-zero TF-IDF scores
            sorted_features = sorted(non_zero_tfidf_scores, key=lambda x: x[1], reverse=True)
